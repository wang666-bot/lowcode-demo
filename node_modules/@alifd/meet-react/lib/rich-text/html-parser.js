    ...
    if (value[0] === '-' && value.lastIndexOf('-') > 0 || value.includes('safe')) {
      ...
      if (value.includes('url')) {
        ...
        value = value.replace(/[0-9.]+\s*rpx/g, function ($) {
          return parseFloat($) * _utils.windowWidth / 750 + "px";
        });
      }
      styleObj[key] = value;
    }
...
HtmlParser.prototype.onOpenTag = function (selfClose) {
  ...
  if (close) {
    ...
    if (node.name === 'img') {
      ...
      if (parseInt(styleObj.width) > _utils.windowWidth) {
...
HtmlParser.prototype.popNode = function () {
  ...
  if (node.name !== 'table' && parseInt(styleObj.width) > _utils.windowWidth) {
      ...
      for (var row = 1; row <= trList.length; row++) {
        ...
        for (var j = 0; j < trList[row - 1].children.length; j++, col++) {
          ...
          if (td.name === 'td' || td.name === 'th') {
            ...
            _style2 += (border ? ";border:" + border + "px solid gray" + (spacing ? '' : ';border-right:0;border-bottom:0') : '') + (padding ? ";padding:" + padding + "px" : '');
            if (td.attrs.colspan) {
              _style2 += ";grid-column-start:" + col + ";grid-column-end:" + (col + parseInt(td.attrs.colspan));
              if (!td.attrs.rowspan) {
                _style2 += ";grid-row-start:" + row + ";grid-row-end:" + (row + 1);
              }
              col += parseInt(td.attrs.colspan) - 1;
            }
            if (td.attrs.rowspan) {
              _style2 += ";grid-row-start:" + row + ";grid-row-end:" + (row + parseInt(td.attrs.rowspan));
              if (!td.attrs.colspan) {
                _style2 += ";grid-column-start:" + col + ";grid-column-end:" + (col + 1);
              }
        ...
        (function traversal(nodes) {
          for (var _i6 = 0; _i6 < nodes.length; _i6++) {
            ...
            if (_td.name === 'th' || _td.name === 'td') {
              if (border) {
                _td.attrs.style = "border:" + border + "px solid gray;" + (_td.attrs.style || '');
              }
              if (padding) {
                _td.attrs.style = "padding:" + padding + "px;" + (_td.attrs.style || '');
              }
  ...
  for (var key in styleObj) {
    if (styleObj[key]) {
      var val = ";" + key + ":" + styleObj[key].replace(' !important', '');
...
HtmlParser.prototype.onText = function (text) {
  ...
  if (this.hook(node)) {
    if (this.options.selectable === 'force' && _utils.isIOS) {
      this.expose();
      node.us = 'T';
    }
    var siblings = this.stack.length ? this.stack[this.stack.length - 1].children : this.nodes;
...
var _default = exports.default = HtmlParser;