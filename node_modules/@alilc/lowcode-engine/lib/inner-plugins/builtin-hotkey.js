...
function insertChild(container, originalChild, at) {
  ...
  if ((0, _lowcodeUtils.isNode)(child) && child.isSlotNode) {
    child = child.exportSchema(_lowcodeTypes.IPublicEnumTransformStage.Clone);
  }
  var node = null;
  if ((0, _lowcodeUtils.isNode)(child)) {
...
function getSuitableInsertion(pluginContext, insertNode) {
  ...
  if (Array.isArray(insertNode) && (0, _lowcodeUtils.isNodeSchema)(insertNode[0]) && (_material$getComponen = material.getComponentMeta(insertNode[0].componentName)) !== null && _material$getComponen !== void 0 && _material$getComponen.isModal) {
    if (!activeDoc.root) {
      return null;
    }
    return {
...
// 注册默认的 setters
var builtinHotkey = exports.builtinHotkey = function builtinHotkey(ctx) {
  return {
    init: function init() {
      ...
      hotkey.bind(['backspace', 'del'], function (e, action) {
        ...
        if ((0, _lowcodeUtils.isFormEvent)(e) || !doc) {
          return;
        }
        e.preventDefault();
      ...
      hotkey.bind('escape', function (e, action) {
        ...
        if ((0, _lowcodeUtils.isFormEvent)(e) || !sel) {
          return;
        }
        e.preventDefault();
        sel.clear();
      ...
      hotkey.bind(['command+c', 'ctrl+c', 'command+x', 'ctrl+x'], function (e, action) {
        ...
        var componentsTree = selected.map(function (item) {
          return item === null || item === void 0 ? void 0 : item.exportSchema(_lowcodeTypes.IPublicEnumTransformStage.Clone);
        });
      ...
      hotkey.bind(['command+z', 'ctrl+z'], function (e, action) {
        ...
        if ((0, _lowcodeUtils.isFormEvent)(e) || !history) {
          return;
        }
        e.preventDefault();
